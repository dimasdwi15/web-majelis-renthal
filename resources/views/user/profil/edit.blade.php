...
@section('content')
    ...
    <div class="max-w-2xl space-y-4">
                ...
                <div class="flex justify-end pt-2" style="border-top: 1px solid rgba(101,94,68,0.08);">
                    <button type="submit" :disabled="loading"
                        ...
                        <span class="material-symbols-outlined text-base animate-spin"
                            x-show="loading">progress_activity</span>
        ...
        <div class="rounded-xl overflow-hidden" x-data="{ loading: false, showCurrent: false, showNew: false, showConfirm: false }"
            ...
            <form method="POST" action="{{ route('user.profil.update-password') }}" @submit="loading = true"
                class="p-4 space-y-4">
                @csrf
                @method('PATCH')

                {{-- Current Password --}}
                @if (!(Auth::user()->isGoogle() && !Auth::user()->hasPassword()))
                    <div>
                        <label class="block text-[9px] uppercase tracking-widest font-semibold mb-1.5"
                            style="color: #7b776c;">Password Saat Ini</label>
                        <div class="relative">
                ...
                {{-- Info badge untuk akun Google yang belum punya password --}}
                @if (Auth::user()->isGoogle() && !Auth::user()->hasPassword())
                    <div class="flex items-start gap-2 p-3 rounded-xl"
                        style="background: #f0fdf4; border: 1px solid #86efac;">
                        <span class="material-symbols-outlined text-sm mt-0.5" style="color: #16a34a;">info</span>
                        <p class="text-xs leading-relaxed" style="color: #15803d;">
                            Akun Anda terhubung via Google. Buat password untuk bisa login menggunakan email/password juga.
                        </p>
                    </div>
                @endif
                ...
                <div>
                    <label class="block text-[9px] uppercase tracking-widest font-semibold mb-1.5"
                        style="color: #7b776c;">
                        Password Baru <span style="color: #a09880; normal-case font-normal text-[9px];">(min. 8
                            karakter)</span>
                    </label>
                    <div class="relative">
                        <input :type="showNew ? 'text' : 'password'" name="password" placeholder="Minimal 8 karakter"
                ...
                <div>
                    <label class="block text-[9px] uppercase tracking-widest font-semibold mb-1.5"
                        style="color: #7b776c;">Konfirmasi Password Baru</label>
                    <div class="relative">
        ...
        <div class="rounded-xl overflow-hidden" x-data="{ confirm: false }"
            ...
            <div class="p-4">
                ...
                <div x-show="confirm" x-transition:enter="transition ease-out duration-200"
                    ...
                    class="mt-3 rounded-xl p-4" style="background: #fef2f2; border: 1px solid #fca5a5;">
                    <p class="text-xs font-black uppercase tracking-widest mb-2" style="color: #991b1b;">⚠ Konfirmasi
                        Penghapusan</p>
                    <p class="text-xs mb-3 leading-relaxed" style="color: #7b776c;">

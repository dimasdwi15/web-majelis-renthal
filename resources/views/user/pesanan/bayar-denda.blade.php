...
@section('content')
    ...
    <div class="max-w-lg mx-auto">
            ...
            <div class="px-4 py-4">
                ...
                @if ($denda->catatan)
                    <div class="rounded-xl px-3 py-2.5 mb-3" style="background: #fef9f0; border: 1px solid #fde68a;">
                        <p class="text-[9px] uppercase tracking-widest font-semibold mb-1" style="color: #92400e;">Catatan
                            dari Admin</p>
                ...
                @if ($denda->foto->count() > 0)
                    <div>
                        <p class="text-[9px] uppercase tracking-widest font-semibold mb-2" style="color: #a09880;">Bukti
                            Kerusakan</p>
                        <div class="flex flex-wrap gap-2">
                            @foreach ($denda->foto as $foto)
                                <a href="{{ Storage::url($foto->path_foto) }}" target="_blank"
                                    ...
                                    <img src="{{ Storage::url($foto->path_foto) }}" alt="Bukti"
        ...
        {{-- Pilih Metode --}}
        <div x-data="{ metode: '{{ old('metode', 'midtrans') }}' }" class="rounded-xl overflow-hidden mb-4"
                ...
                <label class="flex items-center gap-3 p-3.5 rounded-xl cursor-pointer transition-all"
                    ...
                    <div class="flex items-center gap-2.5 flex-1">
                        ...
                        <div>
                            ...
                            <p class="text-[10px] uppercase tracking-wider" style="color: #7b776c;">Transfer · QRIS ·
                                E-Wallet via Midtrans</p>
                        </div>
                    </div>
                ...
                <label class="flex items-center gap-3 p-3.5 rounded-xl cursor-pointer transition-all"
                    ...
                    <div class="flex items-center gap-2.5 flex-1">
                        ...
                        <div>
                            ...
                            <p class="text-[10px] uppercase tracking-wider" style="color: #7b776c;">Bayar langsung di toko
                            </p>
                        </div>
                    </div>
                </label>
            ...
            {{-- Info Tunai --}}
            <div x-show="metode === 'tunai'" x-transition:enter="transition ease-out duration-150"
            ...
            <div class="px-4 pb-4">
                <div class="flex items-center justify-between py-3.5"
                    style="border-top: 1px solid rgba(101,94,68,0.1); margin-bottom: 12px;">
                    <p class="text-xs font-semibold uppercase tracking-wider" style="color: #7b776c;">Total Tagihan</p>
                    <p class="text-xl font-black" style="color: #2f342e;">Rp
                        {{ number_format($denda->jumlah, 0, ',', '.') }}</p>
                </div>
                ...
                <div x-show="metode === 'tunai'" x-transition>
                    <div class="rounded-xl px-4 py-4 text-center"
                        style="background: #f4f2ec; border: 1px solid rgba(101,94,68,0.15);">
...
@push('scripts')
    ...
    <script>
        async function bayarDenda(dendaId) {
            try {
                const response = await fetch(`/bayar-denda/${dendaId}`, {
                    method: 'POST',
                    headers: {
                ...
                snap.pay(data.snap_token, {
                    onSuccess: function(result) {
                        window.location.href = "{{ route('user.pesanan.struk', $denda->transaksi_id) }}";
                    },
                    ...
                    onClose: function() {
                        console.warn('User menutup popup tanpa bayar');
                    }
                });

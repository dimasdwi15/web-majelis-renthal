...
<div class="flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-6">
    ...
    <div class="flex items-center gap-2">
        ...
        {{-- Tombol Struk --}}
        @if(in_array($transaksi->status, ['dibayar', 'berjalan', 'terlambat', 'selesai']))
...
<div class="grid lg:grid-cols-3 gap-4">
    ...
    <div class="lg:col-span-2 space-y-4">
        ...
        <div class="rounded-xl overflow-hidden" style="background: #fff; box-shadow: 0 2px 10px rgba(37,29,29,0.07);">
                        ...
                        <div class="w-14 h-14 rounded-xl overflow-hidden flex-shrink-0" style="background: #f4f2ec;">
                            ...
                            @else
                                <div class="w-full h-full flex items-center justify-center">
                                    <span class="material-symbols-outlined text-xl" style="color: #ccc6b9;">image_not_supported</span>
                                </div>
                            @endif
                        </div>
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-bold truncate" style="color: #2f342e;">{{ $detail->barang->nama }}</p>
                            <p class="text-[10px] uppercase tracking-wider mt-0.5" style="color: #7b776c;">
                                {{ $detail->jumlah }} × Rp {{ number_format($detail->harga_per_hari, 0, ',', '.') }}/hari
            ...
            <div class="px-4 py-3.5 space-y-2">
                <div class="flex justify-between text-xs">
                    ...
                    <span class="font-bold" style="color: #2f342e;">Rp {{ number_format($transaksi->total_sewa, 0, ',', '.') }}</span>
                </div>
                @if($transaksi->total_denda > 0)
                    <div class="flex justify-between text-xs">
                        <span class="uppercase tracking-wider font-semibold" style="color: #dc2626;">Total Denda</span>
                        <span class="font-bold" style="color: #dc2626;">+ Rp {{ number_format($transaksi->total_denda, 0, ',', '.') }}</span>
                    </div>
                @endif
                @if(isset($transaksi->total_charge) && $transaksi->total_charge > 0)
                    <div class="flex justify-between text-xs">
                        <span class="uppercase tracking-wider font-semibold" style="color: #d97706;">Biaya Tambahan</span>
                        <span class="font-bold" style="color: #d97706;">+ Rp {{ number_format($transaksi->total_charge, 0, ',', '.') }}</span>
                    </div>
                @endif
                <div class="flex justify-between pt-2" style="border-top: 1px solid rgba(101,94,68,0.1);">
                    <span class="text-sm font-black uppercase tracking-wider" style="color: #2f342e;">Grand Total</span>
                    <span class="text-base font-black" style="color: #2f342e;">
                        Rp {{ number_format(
                            $transaksi->total_sewa
                            + $transaksi->total_denda
                            + (isset($transaksi->total_charge) ? $transaksi->total_charge : 0),
                        0, ',', '.') }}
                    </span>
                </div>
            </div>
        ...
        <div class="rounded-xl overflow-hidden" style="background: #fff; box-shadow: 0 2px 10px rgba(37,29,29,0.07);">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3">
                <div class="px-4 py-3.5" style="border-right: 1px solid #f4f2ec;">
                    <p class="text-[9px] uppercase tracking-widest font-semibold mb-1" style="color: #a09880;">Tanggal Ambil</p>
                    <p class="text-sm font-black" style="color: #2f342e;">{{ \Carbon\Carbon::parse($transaksi->tanggal_ambil)->format('d M Y') }}</p>
                </div>
                <div class="px-4 py-3.5" style="border-right: 1px solid #f4f2ec;">
                    <p class="text-[9px] uppercase tracking-widest font-semibold mb-1" style="color: #a09880;">Batas Kembali</p>
                    <p class="text-sm font-black" style="color: #2f342e;">{{ \Carbon\Carbon::parse($transaksi->tanggal_kembali)->format('d M Y') }}</p>
                </div>
                @if($transaksi->tanggal_dikembalikan)
                    <div class="px-4 py-3.5">
                        <p class="text-[9px] uppercase tracking-widest font-semibold mb-1" style="color: #a09880;">Dikembalikan</p>
            ...
            {{-- Countdown --}}
            @if(in_array($transaksi->status, ['dibayar', 'berjalan']))
                @php
                ...
                @endphp
                <div class="px-4 py-3" style="border-top: 1px solid #f4f2ec; background: {{ $isLate ? '#fef2f2' : '#fafaf7' }};">
            ...
            @forelse($transaksi->pembayaran as $bayar)
                ...
                <div class="flex items-center gap-3 px-4 py-3.5" style="border-bottom: 1px solid #f4f2ec;">
                    <div class="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0"
                        ...
                        <span class="material-symbols-outlined text-sm" style="color: {{ $payColor }};">{{ $payIcon }}</span>
                    </div>
                    <div class="flex-1 min-w-0">
                    ...
                    <div class="text-right flex-shrink-0">
                        <p class="text-sm font-black" style="color: #2f342e;">Rp {{ number_format($bayar->jumlah, 0, ',', '.') }}</p>
                        <p class="text-[10px] font-black uppercase" style="color: {{ $payColor }};">{{ ucfirst($bayar->status) }}</p>
                    </div>
                </div>
            @empty
        ...
        @if($transaksi->denda->count() > 0)
            <div class="rounded-xl overflow-hidden" style="background: #fff; box-shadow: 0 2px 10px rgba(37,29,29,0.07); border: 1px solid #fecaca;">
                <div class="px-4 py-3 flex items-center gap-2" style="background: #fef2f2; border-bottom: 1px solid #fecaca;">
                    ...
                    <p class="text-xs font-black uppercase tracking-widest" style="color: #991b1b;">Tagihan Denda</p>
                </div>
                @foreach($transaksi->denda as $denda)
                    <div class="px-4 py-4" style="border-bottom: 1px solid #fff0f0;">
                        <div class="flex items-start justify-between gap-3 mb-3">
                            ...
                            <div class="text-right flex-shrink-0">
                                <p class="text-lg font-black" style="color: #dc2626;">Rp {{ number_format($denda->jumlah, 0, ',', '.') }}</p>
                                @if($denda->dibayar_pada)
                                    <span class="inline-flex items-center gap-1 text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded-lg"
                                          style="background: #dcfce7; color: #166534;">
                                        <span class="material-symbols-outlined text-sm">check</span> LUNAS
                                    </span>
                                @else
                                    <span class="inline-flex items-center gap-1.5 text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded-lg"
                                          style="background: #fee2e2; color: #991b1b;">
                                        <span class="w-1.5 h-1.5 rounded-full pulse-live" style="background: #dc2626;"></span>
                                        BELUM LUNAS
                                    </span>
                                @endif
                            </div>
                        ...
                        @if($denda->catatan)
                            <div class="rounded-lg px-3 py-2 mb-3" style="background: #fef9f0; border: 1px solid #fde68a;">
                                <p class="text-[9px] uppercase tracking-widest font-semibold mb-0.5" style="color: #92400e;">Catatan Admin</p>
                                <p class="text-xs" style="color: #78350f;">{{ $denda->catatan }}</p>
                            </div>
                        @endif

                        @if($denda->foto->count() > 0)
                            <div class="mb-3">
                                <p class="text-[9px] uppercase tracking-widest font-semibold mb-2" style="color: #a09880;">Bukti Foto Kerusakan</p>
                        ...
                        @if(!$denda->dibayar_pada)
                            <a href="{{ route('user.pesanan.bayar-denda', $denda->id) }}"
                               class="inline-flex items-center gap-2 w-full justify-center text-white text-xs font-black uppercase tracking-widest py-3 rounded-xl transition-colors hover:opacity-90"
                               style="background: #dc2626;">
                                <span class="material-symbols-outlined text-base">payment</span>
                                Bayar Denda — Rp {{ number_format($denda->jumlah, 0, ',', '.') }}
                            </a>
                        @endif
                    </div>
                @endforeach
    ...
    <div class="space-y-4">
        ...
        <div class="rounded-xl overflow-hidden" style="background: #fff; box-shadow: 0 2px 10px rgba(37,29,29,0.07);">
            ...
            <div class="px-4 py-3 space-y-3.5">
                <div>
                    <p class="text-[9px] uppercase tracking-widest font-semibold mb-0.5" style="color: #a09880;">Metode Pembayaran</p>
                    <p class="text-xs font-bold flex items-center gap-1.5" style="color: #2f342e;">
                        <span class="material-symbols-outlined text-sm" style="color: #655e44;">
                            {{ $transaksi->metode_pembayaran === 'midtrans' ? 'credit_card' : 'payments' }}
                        </span>
                        {{ strtoupper($transaksi->metode_pembayaran) }}
                    </p>
                </div>
                <div>
                    <p class="text-[9px] uppercase tracking-widest font-semibold mb-0.5" style="color: #a09880;">Status Pembayaran</p>
                    @php
        ...
        {{-- Jaminan Identitas --}}
        @if($transaksi->jaminanIdentitas->count() > 0)
            <div class="rounded-xl overflow-hidden" style="background: #fff; box-shadow: 0 2px 10px rgba(37,29,29,0.07);">
                <div class="px-4 py-3" style="background: #f4f2ec; border-bottom: 1px solid rgba(101,94,68,0.08);">
                    <p class="text-xs font-black uppercase tracking-widest" style="color: #2f342e;">Jaminan Identitas</p>
                </div>
                @foreach($transaksi->jaminanIdentitas as $jaminan)
                    <div class="px-4 py-3" style="border-bottom: 1px solid #f4f2ec;">
                        <div class="flex items-center gap-2 mb-0.5">
                            ...
                            <p class="text-xs font-semibold" style="color: #4d4a3e;">{{ $jaminan->jenis_identitas }}</p>
                        </div>
                        @if($jaminan->status === 'dihapus')
                            <p class="text-[10px] uppercase tracking-wider pl-6" style="color: #7b776c;">File sudah dihapus</p>
                        @endif
                    </div>
                @endforeach
            </div>
        ...
        @if(isset($transaksi->qr_code) && $transaksi->qr_code)
            <div class="rounded-xl overflow-hidden" style="background: #fff; box-shadow: 0 2px 10px rgba(37,29,29,0.07);">
                <div class="px-4 py-3" style="background: #f4f2ec; border-bottom: 1px solid rgba(101,94,68,0.08);">
                    <p class="text-xs font-black uppercase tracking-widest" style="color: #2f342e;">QR Code Pengembalian</p>
                </div>
                ...
                <div class="px-4 pb-4 text-center">
                    <p class="text-[9px] uppercase tracking-widest" style="color: #7b776c;">Tunjukkan ke admin saat pengembalian</p>
                </div>
            </div>
        @endif
        ...
        @if($transaksi->status === 'menunggu_pembayaran')
            <div class="rounded-xl p-4" style="background: #fef3c7; border: 1px solid #fcd34d;">
                <p class="text-xs font-black uppercase tracking-widest mb-1" style="color: #92400e;">Menunggu Pembayaran</p>
                <p class="text-xs mb-3 leading-relaxed" style="color: #78350f;">Segera selesaikan pembayaran sebelum pesanan dibatalkan.</p>
                @if($transaksi->metode_pembayaran === 'midtrans')
                    ...
                    @if($pembayaranUtama && $pembayaranUtama->referensi_midtrans)
                        <button onclick="snapPay('{{ $pembayaranUtama->referensi_midtrans }}')"
                    ...
                    @endif
                @else
                    <div class="rounded-xl px-4 py-3 text-center" style="background: #fff8e1; border: 1px solid rgba(101,94,68,0.2);">
                        <span class="material-symbols-outlined text-2xl block mb-1" style="color: #655e44;">store</span>
                        <p class="text-[10px] uppercase tracking-wider" style="color: #7b776c;">Bayar tunai ke admin</p>
                    </div>
                @endif
            </div>
        @endif

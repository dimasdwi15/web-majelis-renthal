...
<body x-data="{ sidebarOpen: false }">
    ...
    <div class="min-h-screen flex">
        ...
        {{-- ===== SIDEBAR ===== --}}
        <aside
            class="fixed top-0 left-0 h-full w-64 z-50 flex flex-col bg-[#1e1714] transition-transform duration-300 -translate-x-full lg:translate-x-0"
            :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'"
            style="border-right: 1px solid rgba(255,255,255,0.04);">

            {{-- Logo --}}
            <div class="flex items-center justify-between px-5 py-5 flex-shrink-0"
                style="border-bottom: 1px solid rgba(255,255,255,0.05);">
                <a href="{{ url('/') }}" class="group flex items-center gap-2.5">
                    <div class="w-8 h-8 flex items-center justify-center rounded-lg"
                        style="background: linear-gradient(135deg, #655e44, #4d4030); border: 1px solid rgba(168,149,106,0.3);">

                        <img src="{{ asset('images/majelis.png') }}" alt="Logo" class="w-5 h-5 object-contain">

                    </div>
                    <div>
                        <p class="text-[#F2E8C6] text-sm font-black tracking-tight leading-none">MAJELIS</p>
                        <p class="text-[10px] font-bold tracking-[0.25em] uppercase" style="color: #655e44;">RENTAL</p>
                    </div>
                </a>
            ...
            <nav class="flex-1 px-3 space-y-0.5 overflow-y-auto scrollbar-hide pb-2">
                ...
                <p class="text-[9px] font-black uppercase tracking-[0.2em] px-3 pt-3 pb-1.5" style="color: #3d3530;">
                    Menu</p>
                ...
                @foreach ($navItems as $item)
                    ...
                    <a href="{{ route($item['route']) }}"
                        ...
                        @if ($item['route'] === 'user.notifikasi.index')
                            ...
                            @if ($unreadCount > 0)
                                <span
                                    class="min-w-[18px] h-[18px] text-[#251D1D] text-[9px] font-black rounded-full flex items-center justify-center px-1"
                ...
                <p class="text-[9px] font-black uppercase tracking-[0.2em] px-3 pt-5 pb-1.5" style="color: #3d3530;">
                    Lainnya</p>

                <a href="{{ url('/') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl nav-item group">
                    <span
                        class="material-symbols-outlined nav-icon text-lg flex-shrink-0 transition-colors">public</span>
                    <span class="text-xs font-bold uppercase tracking-wider">Website Utama</span>
                </a>
                <a href="{{ url('/katalog') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl nav-item group">
                    <span
                        class="material-symbols-outlined nav-icon text-lg flex-shrink-0 transition-colors">explore</span>
        ...
        <div class="flex-1 lg:ml-64 min-w-0 flex flex-col min-h-screen">
            ...
            <header class="sticky top-0 z-30 flex items-center justify-between px-4 py-3 lg:hidden"
                ...
                <a href="{{ route('user.notifikasi.index') }}" class="relative transition-colors"
                    style="color: #7b776c;">
                    <span class="material-symbols-outlined text-xl">notifications</span>
                    @php $unreadCount = \App\Models\Notifikasi::where('user_id', Auth::id())->where('dibaca', false)->count(); @endphp
                    @if ($unreadCount > 0)
                        <span
                            class="absolute -top-1 -right-1 w-4 h-4 text-[#251D1D] text-[8px] font-black rounded-full flex items-center justify-center"
                            style="background: #a8956a;">
                            {{ $unreadCount > 9 ? '9+' : $unreadCount }}
                        </span>
                    @endif

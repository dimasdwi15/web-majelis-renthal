...
<head>
    ...
    <style>
        ...
        .dm-thumbs::-webkit-scrollbar {
            height: 3px;
        }

        .dm-thumbs::-webkit-scrollbar-track {
            background: #e4dfce;
        }

        .dm-thumbs::-webkit-scrollbar-thumb {
            background: #b8b09a;
            border-radius: 3px;
        }
        ...
        .dm-thumb-active {
            border-color: #4d462e !important;
            opacity: 1 !important;
            transform: scale(1.05);
        }

        .dm-thumb:not(.dm-thumb-active) {
            opacity: 0.45;
        }

        .dm-thumb:not(.dm-thumb-active):hover {
            opacity: 0.8;
        }
...
<body class="bg-[#f5f3ed] text-[#1b1c1a] antialiased">
    ...
    <main x-data="{
            ...
            @else
                <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                    @foreach ($barang as $index => $item)
                        @php
                            $allPhotos = collect($item->fotos)
                                ->map(fn($f) => asset('storage/' . $f->path_foto))
                                ->values()
                                ->toArray();
                            if (empty($allPhotos)) {
                                $allPhotos = [
                                    $item->fotoUtama && $item->fotoUtama->path_foto
                            ...
                            $itemData = [
                                ...
                                // seweLangsungUrl: tambah item ke cart sekaligus redirect checkout
                                'seweLangsungUrl' => route('keranjang.sewa-langsung', $item->id),
                        ...
                        {{--
                            ╔═══════════════════════════════════════════════════════════╗
                            ║  FIX: gunakan $store.cart.syncItems(data.cart) bukan      ║
                            ║  $store.cart.items = data.cart secara langsung.            ║
                            ║  Alasan:                                                   ║
                            ║  1. syncItems() menjamin items selalu berupa object {}     ║
                            ║     bukan array [] yang bisa merusak getter 'keys'.        ║
                            ║  2. syncItems() menaikkan version counter sehingga        ║
                            ║     komponen x-for yang bergantung :key="version" ikut     ║
                        ...
                        <div x-data="{
                            ...
                            <div class="card-img-wrap relative overflow-hidden bg-[#eeead8] aspect-[4/3]">
                                ...
                                <div
                                    class="card-overlay absolute inset-0 bg-gradient-to-t from-[#1b1c1a]/30 to-transparent opacity-0 transition-opacity duration-300">
                                </div>
                                ...
                                @if ($item->stok > 0 && $item->stok <= 3)
                                    <div class="absolute top-3 right-3 z-10">
                                        <span
                                            class="font-syne font-bold text-[9px] tracking-[0.1em] uppercase px-2.5 py-1.5 rounded-full bg-amber-500 text-white">Sisa
                                            {{ $item->stok }}</span>
                                    </div>
                                @endif
                                @if (collect($item->fotos)->count() > 1)
                                    <div class="absolute bottom-3 right-3 z-10">
                                        <span
                                            class="flex items-center gap-1 font-syne font-bold text-[9px] tracking-[0.08em] uppercase px-2.5 py-1.5 rounded-full bg-black/50 text-white backdrop-blur-sm">
                            ...
                            <div class="p-5 flex flex-col flex-1">
                                @if ($item->kategori)
                                    <div
                                        class="inline-flex items-center gap-1.5 self-start px-3 py-1 rounded-full bg-[#4d462e]/10 border border-[#4d462e]/20 text-[11px] font-semibold text-[#4d462e] tracking-wide uppercase mb-3">
                                        @if ($item->kategori->ikon)
                                            <span
                                                class="w-4 h-4 flex items-center justify-center">@includeIf('icons.' . $item->kategori->ikon)</span>
                                        @endif
                                ...
                                <button type="button" @click="openDetail({{ Js::from($itemData) }})"
                                    ...
                                    <div class="flex items-center gap-2.5">
                                        <div
                                            class="w-7 h-7 rounded-lg bg-[#4d462e]/10 flex items-center justify-center group-hover:bg-[#4d462e]/20 transition-colors duration-200">
                                            <span
                                                class="material-symbols-outlined text-[14px] text-[#4d462e]">info</span>
                                        </div>
                                        <div class="text-left">
                                            <p
                                                class="font-syne font-bold text-[10px] tracking-[0.1em] uppercase text-[#4d462e] leading-none">
                                                Lihat Detail</p>
                                            <p class="text-[9px] text-[#9b947c] mt-0.5 leading-none">Deskripsi &amp;
                                                Spesifikasi</p>
                                        </div>
                                    </div>
                                    <span
                                        class="material-symbols-outlined text-[16px] text-[#c8bfa8] group-hover:text-[#4d462e] group-hover:translate-x-0.5 transition-all duration-200">chevron_right</span>
                                </button>
                                ...
                                <div class="flex flex-col gap-2 mt-auto">
                                    @if ($item->stok > 0)
                                        {{--
                                            FIX: "Sewa Sekarang" sekarang memanggil route keranjang.sewa-langsung
                                            yang otomatis menambahkan item ke cart lalu redirect ke checkout.
                                            Sebelumnya hanya link ke checkout tanpa menambah item.
                                        --}}
                                        <a href="{{ route('keranjang.sewa-langsung', $item->id) }}"
                                            ...
                                            <span class="material-symbols-outlined text-[14px]">bolt</span>Sewa
                                            Sekarang
                                        </a>
                                        <button type="button" x-on:click="addToCart" :disabled="loading"
                                            ...
                                            :class="added ? 'border-green-500 bg-green-50 text-green-700' :
                                                'border-[#c8bfa8] text-[#2e2a1e] hover:bg-[#2e2a1e] hover:text-white hover:border-[#2e2a1e]'">
                                            <template x-if="loading"><span
                                                    class="w-3.5 h-3.5 border-2 border-current border-t-transparent rounded-full animate-spin"></span></template>
                                            <template x-if="!loading && added"><span
                                                    class="material-symbols-outlined text-[14px]">check_circle</span></template>
                                            <template x-if="!loading && !added"><span
                                                    class="material-symbols-outlined text-[14px]">shopping_bag</span></template>
                                            <span
                                    ...
                                    @else
                                        <div
                                            ...
                                            <span
                                                class="material-symbols-outlined text-[14px]">remove_shopping_cart</span>Stok
                                            Habis
                                        </div>
                                    @endif
                                </div>
                    ...
                    <div class="mt-14 flex flex-col items-center gap-4">
                        ...
                        <nav class="flex items-center gap-2">
                            ...
                            @foreach ($pages as $page)
                                ...
                                @if ($page == $current)
                                    <span
                                        class="w-10 h-10 flex items-center justify-center rounded-xl bg-[#4d462e] text-[#F2E8C6] font-syne font-bold text-[12px]">{{ str_pad($page, 2, '0', STR_PAD_LEFT) }}</span>
                                @else
                                    <a href="{{ $barang->url($page) }}"
                                        class="w-10 h-10 flex items-center justify-center rounded-xl border border-[#e0d9c8] text-[#5a584f] font-syne font-bold text-[12px] hover:bg-[#4d462e] hover:text-[#F2E8C6] hover:border-[#4d462e] transition-all">{{ str_pad($page, 2, '0', STR_PAD_LEFT) }}</a>
                                @endif
        ...
        {{-- ══════════════════════════════════════════════════════════════ --}}
        <div x-show="detailOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-5"
            @keydown.escape.window="closeDetail()">

            {{-- Backdrop --}}
            <div x-show="detailOpen" x-transition:enter="transition ease-out duration-200"
                x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
                x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100"
            ...
            <div x-show="detailOpen" x-transition:enter="transition ease-out duration-300"
                ...
                x-transition:leave-end="opacity-0 scale-95 translate-y-4" class="dm-wrap">

                {{-- Top bar --}}
                <div class="dm-topbar">
                    <div class="flex items-center gap-2.5">
                        <div class="h-px w-5 bg-[#4d462e]"></div>
                        <span class="font-syne text-[9px] font-bold tracking-[0.3em] uppercase text-[#7b6f52]">Detail
                            Gear</span>
                        <template x-if="item">
                        ...
                        <template x-if="item">
                            <span
                                class="hidden sm:inline font-syne font-bold text-[10px] uppercase tracking-wide text-[#4d462e] truncate max-w-[200px]"
                ...
                <div class="dm-body">
                    ...
                    <div class="dm-photo">
                        ...
                        <div class="dm-stage">
                            <template x-if="item">
                                <img :src="item.photos[activePhoto] ?? item.photos[0]" :alt="item.nama"
                                    class="dm-stage-img">
                            </template>
                            ...
                            <template x-if="item && item.photos.length > 1">
                                <button @click.stop="nextPhoto()" class="dm-arrow dm-arrow-r">
                                    <span class="material-symbols-outlined"
                                        style="font-size:17px">chevron_right</span>
                                </button>
                            </template>
                            ...
                            <template x-if="item">
                                <div class="space-y-4">
                                    ...
                                    <div class="flex flex-wrap gap-1.5">
                                        <span
                                            :class="item.stok > 0 ? 'bg-emerald-50 border-emerald-200 text-emerald-700' :
                                            ...
                                            <span class="material-symbols-outlined"
                                                style="font-size:11px">inventory_2</span>
                                            <span x-text="item.stok > 0 ? 'Stok: ' + item.stok : 'Stok Habis'"></span>
                                        </span>
                                        <template x-if="item.stok > 0 && item.stok <= 3">
                                            <span
                                                class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-amber-50 border border-amber-200 text-[10px] font-bold text-amber-700 tracking-wide uppercase">
                                                <span class="material-symbols-outlined"
                                                    style="font-size:11px">warning</span>
                                                Menipis!
                                            </span>
                                        </template>
                                    </div>
                                    ...
                                    <div class="flex items-end gap-2 pb-4 border-b border-[#e0d9c8]">
                                        <span
                                            class="font-syne font-extrabold text-[24px] sm:text-[28px] text-[#4d462e] leading-none"
                                    ...
                                    <template x-if="item.deskripsi">
                                        <div>
                                            <div class="flex items-center gap-1.5 mb-2">
                                                <span class="material-symbols-outlined text-[#4d462e]"
                                                    style="font-size:14px">description</span>
                                                <h4
                                                    class="font-syne font-bold text-[10px] tracking-[0.2em] uppercase text-[#4d462e]">
                                                    Deskripsi</h4>
                                            </div>
                                            <p class="text-[13px] text-[#5a584f] leading-relaxed"
                                    ...
                                    <template x-if="item.spesifikasi">
                                        <div>
                                            <div class="flex items-center gap-1.5 mb-2">
                                                <span class="material-symbols-outlined text-[#4d462e]"
                                                    style="font-size:14px">checklist</span>
                                                <h4
                                                    class="font-syne font-bold text-[10px] tracking-[0.2em] uppercase text-[#4d462e]">
                                                    Spesifikasi</h4>
                                            </div>
                                            <div class="bg-[#f5f3ed] border border-[#e0d9c8] rounded-xl p-3.5">
                                                <template
                                                    x-for="(line,i) in item.spesifikasi.split('\n').filter(l=>l.trim())"
                                                    :key="i">
                                                    <div class="spec-line">
                                    ...
                                    <template x-if="!item.deskripsi && !item.spesifikasi">
                                        <div class="flex flex-col items-center py-10 text-center">
                                            <span class="material-symbols-outlined text-[#e0d9c8]"
                                                style="font-size:48px">info</span>
                                            <p class="text-[12px] font-semibold text-[#c8bfa8] mt-3">Belum ada
                                                deskripsi atau spesifikasi.</p>
                                        </div>
                                    </template>
                        ...
                        <div class="dm-info-footer">
                            <template x-if="item && item.stok > 0">
                                <div class="flex flex-col sm:flex-row gap-2">
                                    {{--
                                        FIX: Gunakan seweLangsungUrl (bukan checkoutUrl) agar item
                                        otomatis ditambahkan ke cart sebelum masuk halaman checkout.
                                    --}}
                                    <a :href="item.seweLangsungUrl"
                                    ...
                                    {{--
                                        FIX: Tombol "Tambah Keranjang" di modal juga menggunakan
                                    ...
                                    <button type="button" x-data="{ busy: false, done: false }"
                                        ...
                                        <template x-if="busy"><span
                                                class="w-3.5 h-3.5 border-2 border-current border-t-transparent rounded-full animate-spin"></span></template>
                                        <template x-if="!busy && done"><span class="material-symbols-outlined"
                                                style="font-size:14px">check_circle</span></template>
                                        <template x-if="!busy && !done"><span class="material-symbols-outlined"
                                                style="font-size:14px">shopping_bag</span></template>
                                        <span
                                            x-text="busy ? 'Menambahkan...' : (done ? 'Ditambahkan!' : 'Tambah Keranjang')"></span>
                                    </button>
                                </div>
                            </template>
                            <template x-if="item && item.stok <= 0">
                                <div
                                    class="flex items-center justify-center gap-2 py-3 bg-[#f0ece0] text-[#9b947c] border border-[#e0d9c8] rounded-xl font-syne font-bold text-[10px] tracking-[0.12em] uppercase cursor-not-allowed">
                                    <span class="material-symbols-outlined"
                                        style="font-size:14px">remove_shopping_cart</span>
                                    Stok Habis
                                </div>
                            </template>
                        </div>

    ...
    <script>
        document.addEventListener('alpine:init', () => {
            ...
            Alpine.store('cart', {
                ...
                get total() {
                    return Object.values(this.items).reduce((s, i) => s + (i.harga || 0) * (i.qty || 0), 0);
                },
                get isEmpty() {
                    return this.count === 0;
                },
    ...
    <div x-show="$store.cart.open" x-transition:enter="transition ease-out duration-300"
        ...
        <div class="flex items-center justify-between px-6 py-5 border-b border-[#655e44]/30 flex-shrink-0">
            <div class="flex items-center gap-3">
                ...
                <span class="bg-[#4d462e] text-[#F2E8C6] text-[10px] font-black px-2 py-0.5 rounded-full min-w-[20px] text-center"
                      x-text="$store.cart.count"></span>
            </div>
            <button @click="$store.cart.open = false" class="text-[#F2E8C6]/40 hover:text-[#F2E8C6] transition-colors p-1 rounded hover:bg-[#655e44]/30">
        ...
        <template x-if="!$store.cart.isEmpty">
            <div class="flex-shrink-0 border-t border-[#655e44]/30 px-6 py-5 space-y-3 bg-[#1a1412]">
                <div class="flex justify-between items-baseline">
                    ...
                    <span class="text-[#a8956a] font-extrabold text-xl" x-text="$store.cart.formatRupiah($store.cart.total)"></span>
                </div>
                <p class="text-[#F2E8C6]/25 text-[9px] uppercase tracking-wider">*Belum termasuk durasi & deposit</p>

                <a href="{{ route('checkout.index') }}" class="w-full block text-center bg-[#4d462e] text-[#F2E8C6] py-3.5 rounded text-[10px] uppercase tracking-[0.2em] font-bold hover:bg-[#655e44] transition-colors">
                    Lanjut ke Checkout
                </a>

                <button @click="$store.cart.kosongkan()" class="w-full text-center text-[#F2E8C6]/25 text-[9px] uppercase tracking-widest hover:text-red-400 transition-colors py-1">
                    Kosongkan Keranjang
                </button>
            </div>
        </template>

...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;700;900&family=Bebas+Neue&display=swap" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined:opsz,wght,FILL,GRAD@24,400,0,0&display=swap" rel="stylesheet">

    {{-- Alpine lewat Vite `resources/js/app.js` --}}
    ...
    <style>
        ...
        /* ── Hero spotlight (mouse glow) ─────────────────── */
        #hero-spotlight {
            position: absolute;
            width: 600px;
            height: 600px;
            border-radius: 50%;
            background: radial-gradient(circle, rgba(242,232,198,0.07) 0%, transparent 70%);
            pointer-events: none;
        ...
        .card-shimmer::before {
            ...
            top: 0; left: -100%;
            width: 60%; height: 100%;
            background: linear-gradient(90deg, transparent, rgba(242,232,198,0.07), transparent);
        ...
        .btn-fill:hover .btn-fill-label { color: #F2E8C6; }

        /* ── Scroll indicator ────────────────────────────── */
        .scroll-line {
            width: 1px; height: 0;
            background: linear-gradient(to bottom, rgba(242,232,198,0.45), transparent);
            animation: growLine 1.2s 1.0s cubic-bezier(0.16, 1, 0.3, 1) forwards;
        }
        @keyframes growLine { to { height: 56px; } }

        .scroll-dot {
            animation: bounceDot 2s 1.3s ease-in-out infinite;
        }
        @keyframes bounceDot {
...
<body
    class="bg-[#faf9f5] font-inter overflow-x-hidden"
    x-data="scrollReveal()"
    x-init="init()"
>
    ...
    ══════════════════════════════════════════════════════════ --}}
    <section
        class="relative h-screen flex items-end pb-16 overflow-hidden grain"
        x-data="heroParallax()"
    ...
    >
        ...
        <div class="absolute top-8 right-8 z-10 hidden md:block">
            <div class="border border-[#F2E8C6]/20 rounded-lg p-4 text-right hero-reveal" style="transition-delay:1.2s">
                <p class="text-[#F2E8C6]/40 text-[9px] uppercase tracking-[0.4em] mb-1">Est. 2025</p>
                <p class="text-[#F2E8C6]/60 text-[10px] font-bold uppercase tracking-widest">Jember, Jawa Timur</p>
            </div>
        </div>

        {{-- Scroll indicator --}}
        <div class="absolute bottom-8 right-8 z-10 hidden md:flex flex-col items-center gap-2 hero-reveal" style="transition-delay:1.4s">
        ...
        <div class="relative z-10 px-6 md:px-16 max-w-screen-2xl w-full mx-auto">
            ...
            <div class="hero-reveal mb-6" style="transition-delay:0.3s">
                <div class="inline-flex items-center gap-3 bg-[#655e44]/80 backdrop-blur-md px-5 py-2 rounded-lg">
                    ...
                    <p class="text-[#F2E8C6] text-[10px] font-bold uppercase tracking-[0.35em]">Deploying Excellence Since 2025</p>
                </div>
            </div>
            ...
            {{-- Bottom Row --}}
            <div class="flex flex-col sm:flex-row items-start sm:items-center gap-6 hero-reveal" style="transition-delay:0.85s">
                <button
                ...
                >
                    <span class="btn-bg absolute inset-0 bg-[#655e44] transition-transform duration-300 -translate-x-full" style="z-index:0"></span>
                    <span class="relative z-10 transition-colors duration-300 group-hover:text-[#F2E8C6]">Explore Repository</span>
                    <span class="material-symbols-outlined relative z-10 text-base transition-all duration-300 group-hover:text-[#F2E8C6] group-hover:translate-x-1">arrow_forward</span>
                </button>
        ...
        <div class="marquee-track">
            @foreach(range(1, 2) as $i)
                @foreach(['Tenda Gunung', 'Carrier Bag', 'Sleeping Bag', 'Sepatu Outdoor', 'Kompor Portable', 'Alat Masak', 'Rain Gear', 'Trekking Pole'] as $item)
                    <span class="text-[#F2E8C6]/70 text-[10px] font-bold uppercase tracking-[0.4em] px-8">{{ $item }}</span>
    ...
    <section class="bg-[#251D1D] py-12 px-6 scroll-reveal">
        <div class="max-w-screen-2xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-0 md:divide-x md:divide-[#F2E8C6]/10">
            @foreach([
                ['100+', 'Unit Tersedia'],
                ['500+', 'Penyewaan'],
                ['5', 'Kategori Gear'],
    ...
    <section class="bg-[#efeeea] py-24 px-6">
        <div class="max-w-screen-2xl mx-auto">
            <div class="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-4 scroll-reveal">
                <div>
                    <p class="text-[10px] font-bold text-[#4d462e]/60 uppercase tracking-[0.4em] mb-3">Technical Categories</p>
                    <h2 class="font-bebas text-7xl md:text-8xl text-[#4d462e] leading-none tracking-wide">Gear Inventory</h2>
                </div>
                <a href="{{ url('/katalog') }}" class="group inline-flex items-center gap-2 text-[#4d462e] text-sm font-bold uppercase tracking-widest border-b border-[#4d462e]/30 pb-1 hover:border-[#4d462e] transition-all">
                    Lihat Semua
                    <span class="material-symbols-outlined text-base group-hover:translate-x-1 transition-transform">north_east</span>
                </a>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <a
                    href="{{ url('/katalog?kategori=' . $slug) }}"
                ...
                >
                    ...
                    <div class="overflow-hidden bg-[#f4f4f0] h-52 relative">
                        ...
                        <div class="absolute top-4 left-4 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-md">
                            <p class="text-[9px] font-bold text-[#4d462e] uppercase tracking-widest">{{ $type }}</p>
                        </div>
                    </div>

                    {{-- Content --}}
                    <div class="p-6">
                        <h3 class="text-xl font-black text-[#4d462e] uppercase tracking-tight mb-2 group-hover:text-[#655e44] transition-colors">{{ $name }}</h3>
                        ...
                        <div class="flex justify-between items-center pt-4 border-t border-[#4d462e]/8">
                            <span class="text-[10px] font-bold text-[#4d462e]/50 uppercase tracking-widest">Lihat Koleksi</span>
                            <div class="w-7 h-7 rounded-full bg-[#efeeea] group-hover:bg-[#655e44] flex items-center justify-center transition-all duration-300">
                                <span class="material-symbols-outlined text-[#4d462e] group-hover:text-[#F2E8C6] text-sm transition-colors">north_east</span>
                            </div>
                        </div>
                    </div>
                </a>
    ...
    <section class="bg-[#faf9f5] py-24 px-6">
        <div class="max-w-screen-2xl mx-auto">
            ...
            <div class="flex flex-col md:flex-row md:items-end justify-between mb-20 gap-8 scroll-reveal">
                <div>
                    <p class="text-[10px] font-bold text-[#4d462e]/50 uppercase tracking-[0.4em] mb-4">Curated Selection</p>
                    <h2 class="font-bebas text-7xl md:text-8xl text-[#4d462e] leading-none tracking-wide">Ready for<br>Deployment</h2>
                </div>
                <div class="bg-[#251D1D] p-6 rounded-xl max-w-xs">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <p class="text-[9px] font-bold text-[#F2E8C6]/40 uppercase tracking-widest mb-1">Quality Assured</p>
                            <p class="text-[#F2E8C6]/80 text-sm italic leading-relaxed">"Semua gear melewati inspeksi teknis 24-titik sebelum tersedia."</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div
                    class="group scroll-reveal card-shimmer bg-[#f4f4f0] rounded-2xl overflow-hidden hover:shadow-2xl hover:shadow-[#4d462e]/15 transition-all duration-500"
                ...
                >
                    ...
                    <div class="relative overflow-hidden h-80">
                        ...
                        />
                        {{-- Overlay on hover --}}
                        <div class="absolute inset-0 bg-[#251D1D]/0 group-hover:bg-[#251D1D]/20 transition-all duration-500"></div>

                        {{-- Status badge --}}
                        <div class="absolute top-5 left-5 flex items-center gap-2 bg-white/95 backdrop-blur-sm px-3 py-1.5 rounded-lg shadow-sm">
                            <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
                            <p class="text-[9px] font-bold text-[#4d462e] uppercase tracking-widest">Tersedia</p>
                        </div>

                        {{-- Price overlay on hover --}}
                        <div class="absolute bottom-5 right-5 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-2 group-hover:translate-y-0">
                            <div class="bg-[#655e44] px-4 py-2 rounded-lg">
                                <p class="text-[#F2E8C6] font-bold text-sm">{{ $price }}<span class="text-[#F2E8C6]/60 text-xs font-normal">{{ $unit }}</span></p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="p-6">
                        <div class="flex justify-between items-start mb-4">
                            <h3 class="text-xl font-black text-[#4d462e] uppercase leading-tight tracking-tight">{{ $name }}</h3>
                            <p class="text-[#4d462e] font-bold text-sm shrink-0 ml-2">{{ $price }}<span class="text-[#4d462e]/50 text-xs font-normal">{{ $unit }}</span></p>
                        </div>

                        {{-- Tags --}}
                        <div class="flex flex-wrap gap-2 mb-6">
                            @foreach($tags as $tag)
                            <span class="bg-white border border-[#4d462e]/10 px-3 py-1 text-[9px] font-bold text-[#4a473d] uppercase tracking-widest rounded-full">{{ $tag }}</span>
                            @endforeach
                        </div>
    ...
    <section class="bg-[#655e44] py-24 px-6 relative overflow-hidden scroll-reveal">
        {{-- Background pattern --}}
        <div class="absolute inset-0 opacity-5" style="background-image: repeating-linear-gradient(45deg, #F2E8C6 0px, #F2E8C6 1px, transparent 0px, transparent 50%); background-size: 24px 24px;"></div>
        ...
        <div class="relative max-w-screen-2xl mx-auto flex flex-col md:flex-row items-center justify-between gap-10">
            <div>
                ...
                <h2 class="font-bebas text-6xl md:text-7xl text-[#F2E8C6] leading-none tracking-wide">Mulai Petualangan<br>Kamu Sekarang</h2>
            </div>
    ...
    {{-- ══════════════════════════════════════════════════════════
         TRUST BAR
    ══════════════════════════════════════════════════════════ --}}
    <section class="bg-[#251D1D] py-16 px-6 scroll-reveal">
        <div class="max-w-screen-2xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-10">
            @foreach([
                ['verified_user', 'Barang Terinspeksi', 'Setiap unit melewati pengecekan 24-titik sebelum diserahkan ke penyewa.'],
                ['schedule',      'Jadwal Fleksibel',   'Pilih tanggal sesuai rencana. Ambil langsung atau atur pengiriman.'],
                ['support_agent', 'Dukungan Penuh',     'Tim kami siap membantu sebelum, selama, dan setelah penyewaan.'],
            ] as $item)
            <div class="flex gap-5 items-start">
                <div class="w-10 h-10 rounded-xl bg-[#655e44]/25 flex items-center justify-center shrink-0 mt-0.5">
                    <span class="material-symbols-outlined text-[#F2E8C6]/60 text-lg">{{ $item[0] }}</span>
                </div>
                <div>
                    <h4 class="text-[#F2E8C6] font-bold text-sm uppercase tracking-wide mb-1.5">{{ $item[1] }}</h4>
                    <p class="text-[#F2E8C6]/38 text-sm leading-relaxed">{{ $item[2] }}</p>
                </div>
            </div>
            @endforeach
        </div>
    ...
    <section class="bg-[#655e44] py-24 px-6 relative overflow-hidden scroll-reveal">
        <div class="absolute inset-0 opacity-[0.04]"
             style="background-image: repeating-linear-gradient(45deg, #F2E8C6 0px, #F2E8C6 1px, transparent 0, transparent 50%);
                    background-size: 22px 22px;"></div>
        <div class="absolute right-0 bottom-0 leading-none select-none pointer-events-none overflow-hidden">
            <span class="font-bebas text-[20vw] text-[#F2E8C6]/5 tracking-wide">DEPLOY</span>
        </div>
        <div class="relative max-w-screen-2xl mx-auto flex flex-col md:flex-row items-center justify-between gap-10">
            <div>
                <p class="text-[#F2E8C6]/45 text-[10px] font-bold uppercase tracking-[0.4em] mb-4">Ready to Deploy?</p>
                <h2 class="font-bebas text-6xl md:text-7xl text-[#F2E8C6] leading-none tracking-wide">Mulai Petualangan<br>Kamu Sekarang</h2>
            </div>
            <div class="flex flex-col sm:flex-row gap-4 shrink-0">
                <a href="{{ url('/katalog') }}"
                   class="group bg-[#F2E8C6] text-[#251D1D] px-8 py-4 rounded-xl font-bold uppercase tracking-widest text-sm flex items-center gap-2 hover:bg-white hover:scale-105 transition-all duration-300">
                    Lihat Katalog
                    <span class="material-symbols-outlined text-base group-hover:translate-x-1 transition-transform">north_east</span>
                </a>
                <a href="#"
                   class="border border-[#F2E8C6]/25 text-[#F2E8C6] px-8 py-4 rounded-xl font-bold uppercase tracking-widest text-sm hover:bg-[#F2E8C6]/10 transition-all duration-300">
                    Hubungi Kami
                </a>
            </div>
        </div>
        ...
        function heroController() {
            return {
                onMouseMove(e) {
                    ...
                    // Parallax: image drifts gently opposite to mouse
                    const cx = (e.clientX / window.innerWidth  - 0.5);
                    const cy = (e.clientY / window.innerHeight - 0.5);
                    img.style.transform = `translate(${cx * 16}px, ${cy * 10}px) scale(1.06)`;

                    // Spotlight follows mouse position within section
